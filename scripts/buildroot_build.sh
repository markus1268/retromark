...
echo "Buildroot building...: retromark-${platform}_defconfig"

# start docker
# check docker image available ?

# run docker container
docker run -it --rm -v ${PROJECT_DIR}:/build -v ${DL_DIR}:/build/buildroot/dl -v ${OUTPUT_DIR}/${platform}:/${platform} -v ${CCACHE_DIR}:/home/${USER0}/.buildroot-ccache -v /etc/passwd:/etc/passwd:ro -v /etc/group:/etc/group:ro -u ${UID0}:${GID0} ${DOCKER_REPO}/${DOCKER_IMAGE} make O=/${platform} BR2_EXTERNAL=/build -C /build/buildroot ${CMD}
